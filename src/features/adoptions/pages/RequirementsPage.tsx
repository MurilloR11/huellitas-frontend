...
// ─── BrandCheckbox ─────────────────────────────────────────────────────────────
// Visually-hidden native input + styled visual div for full brand-color control.
...
function ChecklistItem({
  ...
  return (
    <label
      htmlFor={id}
      className={cn(
        'flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer border select-none',
...
function DocumentItem({ label, note }: { label: string; note: string }) {
  return (
    <div className="flex items-start gap-3 px-4 py-3 rounded-xl border border-stone-100 bg-stone-50">
      <div
...
// ─── StepperBar ────────────────────────────────────────────────────────────────
// Desktop: numbered circles + connector lines + labels.
// Mobile: hidden (replaced by compact dot indicator in page).
      ...
      {STEPS.map((s, i) => {
        ...
        return (
          <Fragment key={i}>
            ...
            {/* Connector line — sits at circle midpoint via mt-[18px] */}
            {i < STEPS.length - 1 && (
              <div className="flex-1 h-0.5 mx-2 mt-[18px] rounded-full overflow-hidden bg-stone-100">
                <div
      ...
      <div className="max-w-xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-5 space-y-4">
          ...
          <div className="flex md:hidden items-center gap-2">
            ...
            <span className="text-xs font-semibold text-stone-700 ml-auto">
              {STEPS[step].short}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between gap-3 pt-3 border-t border-stone-100">
            <p
              className={cn(
                ...
                canProceed ? 'text-green-600' : 'text-stone-500',
              )}
            >
              {summaryText}{canProceed && ' ✓'}
            </p>
            <div className="w-24 h-1.5 rounded-full bg-stone-100 overflow-hidden shrink-0">
              <div
          ...
          <div className="px-5 py-5 min-h-[280px]">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="space-y-2"
              >
                {step === 0 && STEP_1_ITEMS.map((item, i) => (
                  <ChecklistItem
                    key={item}
                    id={`s1-${i}`}
                    label={item}
                ...
                {step === 1 && STEP_2_ITEMS.map((item, i) => (
                  <ChecklistItem
                    key={item}
                    id={`s2-${i}`}
                    label={item}
        ...
        <div className="flex items-center justify-between gap-4 pb-4">
          ...
          <button
            ...
            style={{
              background:    'var(--color-brand)',
              // ring color via CSS custom property isn't possible inline, but focus is fine
            }}
          >
